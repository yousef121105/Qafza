...
def predict_order(order: dict) -> dict:
    ...
    try:
        # Step 4b: reject up front on invalid/out-of-range input, before it
        # ever reaches feature building or the model. See src/validation.py
        # for the Great Expectations suite and the reject-vs-default decision.
        check = validate_order(order)
        if not check["ok"]:
            latency_ms = (time.perf_counter() - start_time) * 1000
            logger.warning(
                "Prediction rejected by validation: %s (latency=%.1fms)",
                check["error"], latency_ms,
            )
            return check
