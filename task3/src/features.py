...
def _validate_order(order: dict) -> None:
    ...
    for field in NUMERIC_FIELDS:
        ...
        if not isinstance(value, (int, float)) or isinstance(value, bool):
            logger.warning(
                "Rejected order: field '%s' is not numeric (got %r)", field, value
            )
        ...
        if value < 0:
            ...
            raise InvalidOrderError(
                f"Field '{field}' cannot be negative, got {value!r}"
            )
...
def _resolve_seller_geography(order: dict) -> dict:
    ...
    same_state = (
        int(order["customer_state"] == primary_seller_state)
        if primary_seller_state
        else 0
    )
...
def _build_date_features(order: dict) -> dict:
    ...
    if estimated_days < 0:
        ...
        logger.warning(
            ...
            "date is before the purchase date",
            estimated_days,
        )
...
def build_features(order: dict) -> pd.DataFrame:
    ...
    raw_df[numeric_features] = ARTIFACTS.numeric_imputer.transform(
    ...
    raw_df[numeric_features] = ARTIFACTS.numeric_scaler.transform(
        raw_df[numeric_features]
    )

    # --- 5. One-hot encode categoricals ---
    # Fitted on train; unseen values -> all zeros.
    encoded = ARTIFACTS.categorical_encoder.transform(raw_df[categorical_features])
    encoded_cols = ARTIFACTS.categorical_encoder.get_feature_names_out(
